...
import com.AIS.Dto.OrderDto;
import com.AIS.Dto.OrderHistDto;
import com.AIS.service.OrderService;
...
public class OrderController {
	...
	public @ResponseBody ResponseEntity order(@RequestBody @Valid OrderDto orderDto,
		...
		Long orderId;
		try {
			orderId = orderService.order(orderDto, email); //분양하기
		} catch (Exception e) {
...
//		model.addAttribute("page", pageable.getPageNumber()); //현재 페이지
	...
	public @ResponseBody ResponseEntity cancelOrder(@PathVariable("orderId") Long orderId,
		Principal principal) {
	//1.분양 취소 권한이 있는지 확인(본인 확인)
		if(!orderService.validateOrder(orderId, principal.getName())) {
			return new ResponseEntity<String>("입약 취소 권한이 없습니다." , HttpStatus.FORBIDDEN);
		}
	//2.분양 취소
	...
	public @ResponseBody ResponseEntity deleteOrder(@PathVariable("orderId") Long orderId,
		...
		//1.본인인증
		if(!orderService.validateOrder(orderId, principal.getName())) {
			return new ResponseEntity<String>("입약기록 삭제 권한이 없습니다." , HttpStatus.FORBIDDEN);
		}
